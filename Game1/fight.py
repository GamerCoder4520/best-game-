...
def move_enemies():
    ...
    for enemy in enemies:
        ...
        if enemy.x > WIDTH or enemy.x <0:
            enemy.vx = -enemy.vx
            animate(enemy, duration=0.1, y=enemy.y + 50)
        for bullet in bullets:
...
def draw_text():
    screen.draw.text("Level " + str(level), (0, 10), color="yellow")
    screen.draw.text("Score " + str(score), (100,0), color="yellow")
